...
const getSecretKeys = async () => {
    try {
        ...
        const configFile = await fs.readFile(`${process.cwd()}/${SECRET_FILE_NAME}`, 'utf8');
        const config = yaml.load(configFile);

        const baseSSMPath = _.get(config, 'env_vars.base_key_path', []);
        const allowedSecretsKeys = _.get(config, 'env_vars.secrets', []);
...
const lambdaWrapper = async (handler) => {
    const keysMap = await getSecretKeys();
